/*
*	Zooz Zen31 LED Controller CT Only
*   v1.0 Place Warm white in the red terminal, cool white in the green terminal. Fixed on/off reporting status, added color temperature commands, all 3 variables (CT, Level, Duration) working.
*
*/
    ...
    if (RGBW_NAMES.every { state.colorReceived[it] != null }) {
        if (device.currentValue("colorMode") == "RGB") {
            ...
            if (wwComponent) {
                ...
                eventProcess(name: "colorTemperature", value: state.setColorTemperatureValue)
            } else {
                // Math is hard
                eventProcess(name: "colorTemperature", value: state.setColorTemperatureValue)
            }
...
void on() {
    ...
    cmds.add(zwave.switchMultilevelV3.switchMultilevelGet())
    sendToDevice(cmds)
    runIn(5, refresh) // Schedule refresh() to run after a 5-second delay
}
...
void setLevel(level, Double duration) {
    // Sets the Level (brightness) of a device (0-99) over a duration (0-???) of (MS?)
    if(logEnable) log.debug "setLevel($level, $duration)"
    if(level > 99) level = 99

    int durationSeconds = (duration * 1000).intValue()
    Integer tt

    if (durationSeconds < 128 * 1000) {
        tt = durationSeconds / 1000
    } else {
        tt = 128 + (durationSeconds / 60000)
    }

    sendToDevice(zwave.switchMultilevelV3.switchMultilevelSet(value: level, dimmingDuration: tt))
     runIn(2, refresh) // Schedule refresh() to run after a 5-second delay
}
...
state.setColorTemperatureValue = null

def setColorTemperature(temp, level = null, Double duration = 1.0) {
    log.debug "setColorTemperature(${temp}, ${level}, ${duration})"
    
    def minTemp = 2000
    def maxTemp = 6500
    def midTemp = (minTemp + maxTemp) / 2
    def redValue = 0
    def greenValue = 0
    
    if (temp <= midTemp) {
        redValue = 255
        greenValue = Math.round(255 * ((temp - minTemp) / (midTemp - minTemp)))
    } else {
        redValue = Math.round(255 * ((maxTemp - temp) / (maxTemp - midTemp)))
        greenValue = 255
    }
    ...
    cmds.add(zwave.switchColorV3.switchColorSet(red: gammaCorrect(redValue), green: gammaCorrect(greenValue), blue: gammaCorrect(0), warmWhite: 0, dimmingDuration: duration))
    state.setColorTemperatureValue = temp
    //cmds.addAll(queryAllColors())
    eventProcess(name: "colorMode", value: "CT")
    eventProcess(name: "colorTemperature", value: temp)
    sendToDevice(cmds)

    // Call setLevel with the input level and duration after setting the color
    setLevel(level, duration)
    runIn(2, refresh) // Schedule refresh() to run after a 5-second delay
}
